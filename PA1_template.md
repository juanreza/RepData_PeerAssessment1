...
Current working directory is c:/edu/_DataScienceJohnsHopkins/_ReproducibleResearch/_ASGN1.
...
There are 61 days with measurement data after filtering out those with missing step measurements.

The average of the totals of steps per day is 9239.490566 steps, ignoring missing data points. By "ignoring" missing values we mean we tell sum to remove NA values. This has the effect of changing NA values to zero, as determined through trial and error.


###B.1. Calculate the daily totals of steps

The total number of steps per day was 570608 steps, ignoring missing data points.
...
The average number of steps per day during all days of the experiment where measurements were present is 37.3826 steps.

The median number of steps per day during all days of the experiment where measurements were present is 0 steps.
...
The 5-minute interval, on average across all the days in the dataset, which contains the maximum number of steps is interval 835-840.
...
The number of intervals without a measurement is 15264.
...
Here are some examples. Given a missing measurement on Saturday at interval 155, which is 1pm, we consider intervals 155 minus 6 thru plus  6  giving intervals 149 thru 161 on that week part (Saturday thru Sunday). Its adjacent day is that Sunday. (If we were looking at a Sunday interval, an adjacent day might be the previous or next weekend date.)  Call this set of values the "related" points for the missing measurement under consideration. If the interval under consideration belongs to Friday, a weekday, its adjacent date is the following Monday.
...
So, incidentally, the experiment was performed on 16 weekend dates and  45 weekend dates.
...
There were 2 weekend and 6 days  missing measurement data, 8 all together.
...
There are now 53 days with measured and estimated step data.
...
The average number of steps per day during all days of the experiment is 37.3826 steps.

The median number of steps per day during all days of the experiment is 0 steps.



###D.4.(c) Do these values differ from the estimates from the first part of the assignment?

The average number of steps including estimated counts is 37.3826 steps.
The average daily steps with and without estimates differ by 0


The median steps including estimated counts is 0 steps.
The median daily steps with and without estimates differ by 0
