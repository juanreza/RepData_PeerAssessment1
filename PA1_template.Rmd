...
See the Supplementary Section for code for [reading the data](#reading)
...
`r totalStepsEachDayHistogram <- function( totalStepsEachDay) { 
  ...
  hh <-  c( dayNumber, totalStepsEachDay)
  hist( hh, col="yellow", oma="2", 
...
# All code used to generate analysis in this paper
```{r echo=TRUE, eval=FALSE}
...
t <- initActivityData ()
 ...
 totalStepsEachDayFunction <- function(t) {
  daysRows <- t[,"date"]
  days <- daysRows[!duplicated(daysRows) ]
...
stepInDays 
 ...
 meanStepsEachDayFunction <- function(t) {
  daysRows <- t[,"date"]
  days <- daysRows[!duplicated(daysRows) ]
  meanStepsEachDay <- sum( t$steps, na.rm=TRUE) / length(days)
  meanStepsEachDay
 ...
 totalStepsEachDayHistogram <- function( totalStepsEachDay) { 
...
#TODO: x is dayNumber
 ...
 meanOfTotalStepsPerDayfunction <- function( totalStepsEachDay ) {
  meanMaxSteps <- mean( totalStepsEachDay , na.rm=TRUE)
meanMaxSteps 
}

 medianOfTotalStepsPerDayfunction <- function( totalStepsEachDay ) {
  medianMaxSteps <- median( totalStepsEachDay , na.rm=TRUE)
medianMaxSteps 
}
 ...
 histClean <- function( tClean ) { # plot
  totalStepsEachDay <- totalStepsEachDayFunction(tClean )
  totalStepsEachDayHistogram( totalStepsEachDay  )
 #  invisible()
}
 ...
 plotMeanStepsByInterval <- function( t) {
 ...
 intervalWithMaxMeanStepsByInterval <- function( t) {
 ...
 intervalWithMaxMeanStepsByIntervalBYMAXOFARRAY <- function( t) {
 ...
 maxMeanStepsByInterval <- function( t) {
...
# for each interval number, get mean of steps in that interval across each day
 plotWeekdayVsWeekendPanelFunction <- function( tClean) {
  tClean$wkt <-  ifelse( strftime( tClean$date, format="%u") %in% c("6","7"), 2, 1)
  ...
  totalStepsEachDay<-totalStepsEachDayFunction(t)
totalStepsEachDay

 meanOfTotalStepsPerDayfunction (  totalStepsEachDay )

 medianOfTotalStepsPerDayfunction (  totalStepsEachDay )

 floor( ((5*806) -2*(60*24)) / 60)

 intervalWithMaxMeanStepsByIntervalBYMAXOFARRAY(t)

 maxMeanStepsByInterval(t)
 totalMissingValues(t)
  tClean <- cleanData(t)
